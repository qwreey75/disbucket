...
client:once('ready', function ()
    ...
    -- raw write message function (included automatic cut string)
    local function rawWriteMessage(str) -- 리밋 레이트 생각 없이 2000 자 제한만 지켜 메시지 쓰기
        ...
        if lastMessage and len(content) <= maxLength then
    ...
    local function writeMessage(str)
        ...
        if messageMutex:isLocked() then -- 만약 쓰기가 진행중이면 buffer 에 집어넣는다
            ...
            if lenbuffer == 0 then -- 버퍼가 비어있다면, 버퍼를 셋업하고 버퍼 비우기를 예약한다
                buffer[1] = str
                messageMutex:lock()
                promise.spawn(bufferClear,buffer,messageMutex.unlock,messageMutex)
                ...
                if len((messageFormat):format((lenbuffer == 1 and lastStr or "") .. newbuffer)) > maxLength-1 then
    ...
    waitter:add(promise.new(function ()
        for str in process.stdout.read do
            for line in str:gmatch"(.-\n)" do
                promise.spawn(writeMessage,line)
            end
            local last = str:match("\n(.-)$")
            if last and last ~= "" then
                promise.spawn(writeMessage,last)
            end
        end
    end))
    waitter:add(promise.new(function ()
